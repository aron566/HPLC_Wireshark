...
**时间**:帧内 NTB 为 **u32,40 µs/tick**。时间轴按 NTB 差推进:首帧(或每段)
frame_time = 其前 8B BCD 标注时刻,后续帧 = 上一帧 frame_time + (本帧 NTB −
上一帧 NTB)×40 µs。**帧间 NTB 差超出合理范围(≤60 s,长时间无报文/跨 u32
回绕)时,导出端在此帧前再写入一次 8B BCD 标注开启新段**;导入端支持文件
中出现**多个 8B 标注**(帧间/头部均可),断段后首帧用新标注、丢弃上一帧 NTB。
