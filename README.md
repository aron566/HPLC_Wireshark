...
标签的 bin 恢复每帧原始捕获时刻(Time 列),Delta 用帧 ts——自 v1.0.12 起
导出 ts 为 **NTB tick**(40 µs 分辨,与实时一致);v1.0.11 及更早导出 bin
的 ts 为 epoch ms 低 32 位(仅毫秒级),请重新导出。
...
| 字段 | 字节 | 说明 |
|------|------|------|
| ts | 0-3 | 帧 **NTB tick**(25 kHz 同步计数,40 µs/tick),小端序 |
| phr_mcs | 4 | 物理层调制编码参数 |
| option | 5 | 物理层 option |
| channel | 6 | 信道编号 |
...
**时间语义**:ts 是 STA 网络内的 NTB tick,回放时同网络相邻帧的
Delta 即 tick 差(40 µs 分辨,与实时一致);裸文件**不携带绝对时刻**,
Time 列显示本地回放时刻(v1.0.11 及更早导出文件 ts 为 epoch ms 低
32 位,不能与 NTB 文件混读,请用新版本重新导出)。导出路径:
菜单 `捕获 → 导出...` → 文件类型选「裸 hex 文本 (*.txt)」。
